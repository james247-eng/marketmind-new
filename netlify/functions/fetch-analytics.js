...
async function fetchProvider(platform, account, token) {
  if (platform === 'facebook') return (await axios.get(`https://graph.facebook.com/v18.0/${account.accountId}/insights`, { params: { metric: 'page_impressions,page_engaged_users,page_post_engagements,page_fans', period: 'day', access_token: token } })).data.data;
  if (platform === 'instagram') return (await axios.get(`https://graph.facebook.com/v18.0/${account.accountId}/insights`, { params: { metric: 'impressions,reach,profile_views,follower_count', period: 'day', access_token: token } })).data.data;
